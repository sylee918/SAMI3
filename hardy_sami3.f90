...
!    subroutine hardy (nfl,nll,blats,alts,blons)
    ...
    subroutine hardy_sami3 (hrut,nfl,nll,iday)
      ...
      integer :: nzh,nfl,nll,j,k,ii,ngrids,is,iit,nglam,ngridn,in,iday
...
!   READ THE DATA ON FIRST PASS
    ...
    IF(IFIRST == 0)THEN
        IFIRST=1
        OPEN(21,FILE='elekp.inp',FORM='FORMATTED')
        DO 10 JOPT=1,2
            DO 10 JKP=1,7
                READ(21,100,ERR=98)LINE
                100 FORMAT(A80)
                DO 10 JCOEF=1,17
                    READ(21,*,ERR=98)JNUM,CR0(JOPT,JKP,JCOEF), &
                    CH0(JOPT,JKP,JCOEF), &
