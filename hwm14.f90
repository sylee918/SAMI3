...
module dwm
    ...
    character(128), parameter  :: dwmdefault = 'dwm07b104i.inp'
...
!   changed per Doug (3/16/19)

    if (input(1) .ne. previous(1) .or. input(2) .ne. previous(2)) then
        AA = (input(1) + input(2)/86400.0)*twoPi/365.25d0 
        do s = 0,MAXS
            BB = dble(s)*AA
            fs(s,1) = dcos(BB)
            fs(s,2) = dsin(BB)
...
contains
    ...
    subroutine initgd2qd()
        ...
        character(128), parameter   :: datafile='gd2qd.inp'
        integer(4)                  :: iterm, n
        integer(4)                  :: j
