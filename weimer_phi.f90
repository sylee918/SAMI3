...
!
    ...
    do j=1,csize
      ...
      if (ab(j)==1) then
       ...
       !if (mlt .eq. 0 .and. lat .eq. 85.47714) print *, colat,plm
       !if (mlt .eq. 0 .and. lat .eq. 86.72631) print *, colat,plm
...
!         j,index,plmtable(1:tablesize,j)
    ...
    !!if (colat .lt. 0.332906575352760) then
    !!!!if ( abs(colat-0.92631) .lt. 1.e-3 ) then
    !! print *, colat, out(1), index
    !! print *
    !! if (index .eq. 1) then
    !!   print *, colattable(1:tablesize)
    !!   print *
    !!   print *, plmtable(1:tablesize,index)
    !! endif
    !!endif
...
!  integer,parameter :: nlat=101, nlon=1+24*4
  ...
  real :: mlat(nlat),mlon(nlon),mlat_pole
  real :: lon(nlon),mlt(nlon),mlt_pole
  ...
  character(len=80) :: datfile="tmp/wei05sc_epot"
  character(len=80) :: idlfile
  ...
  !! open output file for sami3
  open(unit=1101,file='phi_weimer.inp',form='unformatted')
  open(unit=1102,file='fac_weimer.inp',form='unformatted')
  open(unit=1103,file='phi_pole.inp',form='unformatted')
  ...
  open(122,file='weimer_data.inp',form='formatted')
!  read(122,*) line
!  read(122,*) line
!  read(122,*) line
!  read(122,*) line
...
!      print *,i,mlon(i),mlt(i)
    ...
    !! calculate potential at pole
    mlat_pole = 90.
    mlt_pole  =  0.
    call epotval(mlat_pole,mlt_pole,fill,epot_pole)
!      print *,'i,epot_pole = ',itime,epot_pole
    ...
    !!!! plotdat.pro
    !!lu = 20
    !!write (idlfile, "(A,I0.4,A)") trim(datfile), itime, '.dat'
    !!open(lu,file=idlfile,status='replace')
    ...
    !!write(lu,"(6e12.4)") fac
    !!close(lu) 
    !!write(6,"('Wrote ascii file ',A, 2E10.3)") idlfile, maxval(fac), minval(fac)
    ...
    !  close(101)
     ...
     write(1101) hrutw
     write(1101) phi_weimer * 1.e3 / 300. ! convert to statvolt for sami3
     write(1103) epot_pole * 1.e3 / 300.  ! convert to statvolt for sami3
     write(1102) hrutw
     write(1102) fac_weimer

!     print *,'itime,hrut',itime,hrutw,phi_weimer(nlat-6,nlon/2)
